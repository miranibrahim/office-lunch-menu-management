...
const UpdateMenu = () => {
  ...
  const [selectedTags, setSelectedTags] = useState(menuData.data.menu || []);


  useEffect(() => {
    setSelectedTags(menuData.data.menu);
  }, [menuData, reset]);
  ...
  const onSubmit = async (formData) => {
    ...
    const menuItems = {
      date: data.date || testDate.date,
      menu: data.menu,
    };
    console.log(menuItems);
    try {
      const menuRes = await axiosPublic.patch(`/menus/${testDate.date}}`, menuItems);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg">
      <h2 className="text-3xl text-center font-semibold mb-8">Update Menu</h2>
      <h2 className="text-3xl text-center font-semibold mb-8">{testDate.date}</h2>
